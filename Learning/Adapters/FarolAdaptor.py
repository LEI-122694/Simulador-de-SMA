...
class FarolAdapter(TaskAdapter):
    ...
    def is_terminal(self, agent, obs, env):
        # either sensor says HERE, or agent flagged as done
    ...
    def reward(self, agent, prev_state, action, new_state, obs, step, max_steps):
        """
        Same reward shaping as your calcula_recompensa in LighthouseQLearningAgent,
        but written in adapter form.
        """
        # track visited cells on the agent
        ...
        if prev_state is not None:
            ...
            if action in DIRECAO_MAP.get(direction_now, []):
                r += 1.0       # moved in a direction consistent with sensor
