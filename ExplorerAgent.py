...
class ExplorerAgent:
    ...
    def __init__(self, name, env, start_pos):
        ...
        # Store recent positions (for debugging, no loop logic)
    ...
    def comunica(self, mensagem, de_agente):
        ...
        if kind == "goal":
            ...
            self.planned_path = []  # clear old plan
    ...
    # --------------------------- FAROL MODE (NO LOOP DETECTION) --------------------------- #

    def _farol_move(self):
        """
        Pure FAROL logic, no loop detection:
        ...
        if self.env.is_valid_position(*main):
            print(f"   [{self.name}] Rule 1 → direct {main}")
            return main
        print(f"   [{self.name}] Rule 1 blocked {main}")

        # Rule 2 — pure vertical/horizontal attempts
        components = []
        if dx != 0:
        ...
        for c in components:
            if self.env.is_valid_position(*c):
                print(f"   [{self.name}] Rule 2 → sliding {c}")
                return c
            print(f"   [{self.name}] Rule 2 blocked {c}")

        # Rule 3 — random move among valid
        print(f"   [{self.name}] Rule 3 → random")
        neighbors = [
            (self.x - 1, self.y),
            (self.x + 1, self.y),
            (self.x, self.y - 1),
        ...
        # Rule 4 — nowhere to go
        print(f"   [{self.name}] ❌ No valid moves")
        return None

    # --------------------------- BFS for communicated goal --------------------------- #
    ...
    def _follow_or_plan_bfs(self, goal):
        ...
        if not self.planned_path:
            print(f"   [{self.name}] ⚠ BFS failed, fallback")
        ...
        print(f"   [{self.name}] BFS → {next_step}")
        return next_step
    ...
    def _maze_move(self):
        ...
        # Priority: known goal → greedy movement
        target = None
        if self.known_goals:
        ...
        # Otherwise: explore random directions
